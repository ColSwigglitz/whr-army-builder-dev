(() => {
  ...
  async function ensureSingleLandmass() {
    ...
    const offshore = components.slice(1).flat();
    if (!offshore.length) return false;

    realmBusy = true;
    try {
      for (const hex of offshore) {
        const { error } = await window.whrSupabase
          .from('mighty_empire_hexes')
          .update({
            terrain_type:'sea',
  ...
  function hexDistance(a,b) {
    ...
    const ax = a.q, az = ar, ay = -ax-az;
    const br = b.r - (b.q - (b.q & 1)) / 2;
    const bx = b.q, bz = br, by = -bx-bz;
  ...
  function chooseCapitals(hexes,count) {
    ...
    if (count===1) {
      return [candidates.slice().sort((a,b)=>(Math.abs(a.q-centre.q)+Math.abs(a.r-centre.r))-(Math.abs(b.q-centre.q)+Math.abs(b.r-centre.r)))[0]];
    }
    ...
    while(chosen.length<count){
      ...
      remaining.sort((a,b)=>{
        const ad=Math.min(...chosen.map(c=>hexDistance(a,c)));
        const bd=Math.min(...chosen.map(c=>hexDistance(b,c)));
  ...
  async function setupStartingRealms() {
    ...
    const confirmed = window.confirm(
      'Set up starting realms for every current campaign member?\n\nExisting capital markers and Mighty Empires hex ownership will be reset. Each player will receive a well-spaced capital and the adjacent land hexes.'
    );
    if (!confirmed) return;
    ...
    try {
      ...
      if(components.length>1){
        ...
        for(const hex of components.slice(1).flat()){
          const {error}=await window.whrSupabase.from('mighty_empire_hexes').update({terrain_type:'sea',settlement_type:null,owner_id:null,updated_at:new Date().toISOString()}).eq('id',hex.id);
          if(error)throw error;
        }
      }
      ...
      const capitals=chooseCapitals(freshHexes,members.length);
      ...
      const byKey=new Map(freshHexes.map(h=>[key(h.q,h.r),h]));
      ...
      for(let i=0;i<members.length;i++){
        ...
        for(const [nq,nr] of neighbours(capital.q,capital.r)){
          const h=byKey.get(key(nq,nr));
          if(h&&h.terrain_type!=='sea')realm.push(h);
        }
        for(const h of realm){
          ...
          const {error}=await window.whrSupabase
            ...
            .update({
              ...
              settlement_type:h.id===capital.id?'capital':null,
              updated_at:new Date().toISOString()
            })
            .eq('id',h.id);
          if(error)throw error;
  ...
  function installStartingRealmsButton(){
    const dialog=document.getElementById('mightyEmpiresCampaignDialog');
    const actions=dialog?.querySelector('.me-header-actions');
    if(!dialog||!actions||!campaignOwnerIsViewing()||!mapExists()){
  ...
  document.addEventListener('click',event=>{
    if(event.target.closest?.('#meReload'))setTimeout(installControls,0);
  });
})();
